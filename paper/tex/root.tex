...
\begin{figure}[thpb]
	...
	\caption{OpenDRIVE Output of two concatenated T-junctions. Both junctions are almost identically defined. The only differences for the upper junction are that the radius of the main road is changed and an option that automatically generates lane widening for left turns is activated}
	\label{fig_res3_2}
\end{figure}
\begin{table}[h]
...
\hline
XJunc (Fig.~\ref{fig_motivation}) & $875$ & $32468$& $4\%$ &$4.1\%$\\
TwoTJunc (Fig.~\ref{fig_res3_2}) & $937$ & $36414$& $5\%$ &$5.3\%$\\
HugeRoadNet (Fig.~\ref{fig_resHuge}) & $6301$ & $242803$& $3.9\%$ &$4.8\%$
\end{tabular}
\end{center}
\end{table}
