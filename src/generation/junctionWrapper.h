...
int junctionWrapper(pugi::xml_node &node, roadNetwork &data)
{
    ...
    if ((string)node.attribute("type").value() == "M2A")mode = 1;
    if ((string)node.attribute("type").value() == "4A") mode = 1;
    if ((string)node.attribute("type").value() == "MA") mode = 2;
    if ((string)node.attribute("type").value() == "3A") mode = 2;
    
    if (mode == 1) xjunction(node, data);
    if (mode == 2) tjunction(node, data);
    if (mode == 0) 
    {
